...
import { deleteitem, resetItem, incrementquantity, decrementquantity } from '../redux/amazonSlice';
const Cart = () => {
    ...
    return (
        <div className='w-full bg-gray-100 p-4'>
                                    ...
                                    products.map((item) => (
                                        <div key={item.id} className='w-full border-b-[1px] border-b-gray-300 p-4 md:p-0 md:py-4 flex items-center gap-6'>
                                            <div className='w-full flex items-center  gap-6 justify-between'>
                                                <div className='w-1/5'>
                                                    <img className='w-full h-44 object-contain' src={item.image} alt='Productimages'></img>
                                                </div>
                                                <div className='w-4/5'>
                                                    <h2 className='font-semibold text-lg'>{item.title}</h2>
                                                    <p className=' text-sm'>{item.description.substring(0, 220)}</p>
            ...
            '>
                                <button onClick={() => dispatch(resetItem())} className='px-10 py-2 bg-red-500 hover:bg-red-600 active:bg-red-500 text-white rounded-lg font-titleFont font-semibold text-lg tracking-wide'>Clear Cart</button>
                            </div>
                        </div>
                        <div className='w-full h-52 flex flex-col items-center justify-center p-4 bg-white col-span-1'>
                            <div>
                                <p className='flex gap-2 items-start text-sm'><span><CheckCircleIcon className='bg-white text-green-500 rounded-full' /></span>{" "}Your order qualifies for FREE Shipping Choose this option at checkout. See details....</p>
                            </div>
                            <div>
                                <p className="font-semibold px-6 py-1 flex items-center justify-between gap-1">Total:<span className='font-bold text-lg'>${totalprice}</span></p>
                            </div>
                ...
                ) : <motion.div initial={{ y: 70, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.5, duration: 0.5 }} className=' hidden:sm  md:flex justify-center items-center gap-4 py-10   '>
                    <div>
                        <img className='w-80 rounded-lg p-4 mx-auto' src={emptyCart} alt='emptycart'></img>
                    </div>
